...
const invalidInputs = [
  ...
  ['runtime product root', async w => { w.manifest.members[1].skillRoots = ['runtime-skills']; await write(w.checkoutRoot, 'config/developer-workspace.json', JSON.stringify(w.manifest)); }, /Invalid canonical skill roots/],
...
const invalidOwnership = [
  ...
  ['product source', entry => { entry.member = 'cats-runtime'; entry.source = 'runtime-skills/a2a-handoff'; }, /Invalid canonical ownership source/],
  ['unrecognized retired source', entry => { entry.member = 'cats-runtime'; entry.source = 'developer-skills/a2a-handoff'; }, /Invalid canonical ownership source/],
